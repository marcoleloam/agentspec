...
**Framework-Agnostic:** New agents added to `agents/` automatically become available for matching - zero configuration.

> **Note:** The agents shown above (`@backend-developer`, `@data-modeler`, etc.) are hypothetical examples. Your actual agents are discovered from `agents/`.
...
Domains are registered in `kb/_index.yaml` and follow the standard structure documented in `kb/README.md`.
...
touch agents/data-engineering/dbt-specialist.md
```
...
3. **The agent is automatically discoverable** - Design phase will find it via `Glob(agents/**/*.md)`
...
```bash
mkdir -p kb/dbt
touch kb/dbt/{index.md,quick-reference.md}
mkdir -p kb/dbt/{concepts,patterns}
```
...
/define sdd/features/BRAINSTORM_NOTIFICATION_SYSTEM.md

# Phase 2: Design with Agent Matching
/design sdd/features/DEFINE_NOTIFICATION_SYSTEM.md

# Phase 3: Build with Agent Delegation
/build sdd/features/DESIGN_NOTIFICATION_SYSTEM.md

# Phase 4: Archive
/ship sdd/features/DEFINE_NOTIFICATION_SYSTEM.md
```
