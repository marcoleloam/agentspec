...
<!-- Source: .claude/commands/workflow/eval.md -->
...
```markdown
read_file(.claude/sdd/features/DEFINE_{FEATURE}.md)        → acceptance tests
read_file(.claude/sdd/features/DESIGN_{FEATURE}.md)        → ## Evals contract (read-only)
read_file(.claude/sdd/reports/BUILD_REPORT_{FEATURE}.md)   → context only, never evidence
```
