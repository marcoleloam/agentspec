...
**Architect agents** (8 agents in `${GROK_PLUGIN_ROOT}/agents/`):
...
**Cloud agents** (10 agents in `${GROK_PLUGIN_ROOT}/agents/`):
...
**Platform agents** (6 agents in `${GROK_PLUGIN_ROOT}/agents/`):
...
**Data engineering agents** (15 agents in `${GROK_PLUGIN_ROOT}/agents/`):
...
**Python agents** (6 agents in `${GROK_PLUGIN_ROOT}/agents/`):
...
**Test agents** (3 agents in `${GROK_PLUGIN_ROOT}/agents/`):
...
```bash
mkdir -p ${GROK_PLUGIN_ROOT}/kb/{domain}/{concepts,patterns}
```

2. Copy templates from `_templates/`:

```bash
cp ${GROK_PLUGIN_ROOT}/kb/_templates/index.md.template ${GROK_PLUGIN_ROOT}/kb/{domain}/index.md
cp ${GROK_PLUGIN_ROOT}/kb/_templates/quick-reference.md.template ${GROK_PLUGIN_ROOT}/kb/{domain}/quick-reference.md
cp ${GROK_PLUGIN_ROOT}/kb/_templates/concept.md.template ${GROK_PLUGIN_ROOT}/kb/{domain}/concepts/{name}.md
cp ${GROK_PLUGIN_ROOT}/kb/_templates/pattern.md.template ${GROK_PLUGIN_ROOT}/kb/{domain}/patterns/{name}.md
```
...
The machine-readable registry lives at `${GROK_PLUGIN_ROOT}/kb/_index.yaml`. It defines:
...
- **domains** -- Complete registry of all 22 domains with:
  ...
  - `path` -- Directory path relative to `${GROK_PLUGIN_ROOT}/kb/`
