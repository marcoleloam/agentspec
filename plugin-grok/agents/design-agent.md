...
tier: T2
tools: [read_file, write, search_replace, grep, list_dir, run_terminal_command, todo_write, web_search]
kb_domains: []
...
<!-- Source: .claude/agents/workflow/design-agent.md -->
...
│     └─ Read: ${GROK_PLUGIN_ROOT}/kb/{domain}/patterns/*.md → Code patterns      │
│     └─ Read: ${GROK_PLUGIN_ROOT}/kb/{domain}/concepts/*.md → Best practices     │
│     └─ Read: ${GROK_PLUGIN_ROOT}/kb/{domain}/quick-reference.md → Quick lookup  │
│                                                                      │
...
1. Glob `${GROK_PLUGIN_ROOT}/agents/*.md` to discover agents
...
```python
# Pattern: Handler structure (from ${GROK_PLUGIN_ROOT}/kb/{domain}/patterns/{pattern}.md)
