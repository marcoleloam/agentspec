...
+------------------+         |          DESIGN              |
                             |                              |
                             |  Agent Matching:             |
                             |  list_dir(${GROK_PLUGIN_ROOT}/agents/**)     |
                             |         |                    |
                             |         v                    |
                             |  +--------------------+      |
                             |  | Capability Index   |      |
...
list_dir(.claude/           agents:                 stg_orders.sql -> @dbt-specialist
...
**Framework-Agnostic:** New agents added to `${GROK_PLUGIN_ROOT}/agents/` automatically become available for matching -- zero configuration.

### 3. Agent Delegation (Build Phase)

Build invokes matched specialists via the spawn_subagent tool:
...
|  |                   PARALLEL EXECUTION                    | |
|  |                                                         | |
|  |  spawn_subagent(subagent: "dbt-specialist", prompt: "...")       | |
|  |  spawn_subagent(subagent: "spark-engineer", prompt: "...")       | |
|  |  spawn_subagent(subagent: "test-generator", prompt: "...")       | |
|  +--------------------------------------------------------+ |
|                          |                                   |
|                          v                                   |
|  BUILD_REPORT:                                               |
...
Domains are registered in `${GROK_PLUGIN_ROOT}/kb/_index.yaml`. Use `/create-kb` to add new domains.
...
```bash
# Location: ${GROK_PLUGIN_ROOT}/agents/{agent-name}.md
touch ${GROK_PLUGIN_ROOT}/agents/iceberg-specialist.md
```
...
3. **The agent is automatically discoverable** -- Design phase will find it via `list_dir(${GROK_PLUGIN_ROOT}/agents/*.md)`
...
```bash
mkdir -p ${GROK_PLUGIN_ROOT}/kb/iceberg/{concepts,patterns}
touch ${GROK_PLUGIN_ROOT}/kb/iceberg/{index.md,quick-reference.md}
```

3. **Register in KB index** (`${GROK_PLUGIN_ROOT}/kb/_index.yaml`):

```yaml
domains:
  iceberg:
    description: "Apache Iceberg table format patterns"
    entry_point: "${GROK_PLUGIN_ROOT}/kb/iceberg/index.md"
```
...
| Resource | Location |
|----------|----------|
| SDD Index | `${GROK_PLUGIN_ROOT}/sdd/_index.md` |
| Architecture | `${GROK_PLUGIN_ROOT}/sdd/architecture/ARCHITECTURE.md` |
| Workflow Contracts | `${GROK_PLUGIN_ROOT}/sdd/architecture/WORKFLOW_CONTRACTS.yaml` |
| Templates | `${GROK_PLUGIN_ROOT}/sdd/templates/` |
| Archive | `.claude/sdd/archive/` |
| Agents (58) | `${GROK_PLUGIN_ROOT}/agents/` |
| Knowledge Base (22) | `${GROK_PLUGIN_ROOT}/kb/` |
| SDD Commands | `${GROK_PLUGIN_ROOT}/commands/` |
| DE Commands | `${GROK_PLUGIN_ROOT}/commands/` |
| Core Commands | `${GROK_PLUGIN_ROOT}/commands/` |
