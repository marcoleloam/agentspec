...
1. Read every template in `${GROK_PLUGIN_ROOT}/kb/_templates/`.
2. Read `${GROK_PLUGIN_ROOT}/kb/_index.yaml`: the registry format, the file-size limits (the single source of truth for line budgets), and the shape of a domain entry.
3. Read one or two existing domains under `${GROK_PLUGIN_ROOT}/kb/` end to end to see how concepts, patterns, specs, and entry points are actually written in this repository.
...
**Output:** the complete domain under `${GROK_PLUGIN_ROOT}/kb/<domain>/`.
...
Update the aggregator `${GROK_PLUGIN_ROOT}/kb/_index.yaml` **additively**:
