...
<!-- Source: .claude/commands/workflow/ship.md -->
...
```bash
"${AGENTSPEC_PYTHON:-python3}" "${AGENTSPEC_SCRIPTS:-${GROK_PLUGIN_ROOT}/scripts}/eval_runner.py" verify {FEATURE}
```
...
read_file(.claude/sdd/features/DESIGN_{FEATURE}.md)
read_file(.claude/sdd/reports/BUILD_REPORT_{FEATURE}.md)
read_file(.claude/sdd/reports/EVAL_{FEATURE}.json)      # verdict, waivers, legacy_waiver
...
- Agent: `${GROK_PLUGIN_ROOT}/agents/ship-agent.md`
- Template: `${GROK_PLUGIN_ROOT}/sdd/templates/SHIPPED_TEMPLATE.md`
- Contracts: `${GROK_PLUGIN_ROOT}/sdd/architecture/WORKFLOW_CONTRACTS.yaml`
- Previous Phase: `${GROK_PLUGIN_ROOT}/commands/build.md`
