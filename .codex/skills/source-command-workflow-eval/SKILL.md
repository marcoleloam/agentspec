...
Delegate to the **eval-agent** (`.claude/agents/workflow/eval-agent.md`). The agent:
...
```bash
"${AGENTSPEC_PYTHON:-python3}" "${AGENTSPEC_SCRIPTS:-scripts}/eval_runner.py" run {FEATURE}
```
...
```bash
"${AGENTSPEC_PYTHON:-python3}" "${AGENTSPEC_SCRIPTS:-scripts}/eval_runner.py" attest {FEATURE} --eval {id} --verdict pass --owner "{name}" --evidence "{what was checked}" --recorded-via eval-agent
"${AGENTSPEC_PYTHON:-python3}" "${AGENTSPEC_SCRIPTS:-scripts}/eval_runner.py" waive {FEATURE} --eval {id} --supervisor "{name}" --reason "{why}"
"${AGENTSPEC_PYTHON:-python3}" "${AGENTSPEC_SCRIPTS:-scripts}/eval_runner.py" run {FEATURE}
```
...
- Agent: `.claude/agents/workflow/eval-agent.md`
- Runner: `scripts/eval_runner.py`, JEV client: `scripts/jev_client.py`
- Template: `.claude/sdd/templates/EVAL_REPORT_TEMPLATE.md`
- Contracts: `.claude/sdd/architecture/WORKFLOW_CONTRACTS.yaml`
- Previous: `.claude/commands/workflow/build.md` · Next: `.claude/commands/workflow/ship.md`
