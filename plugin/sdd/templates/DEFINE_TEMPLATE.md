...
| **Domínios KB** | {Liste os domínios de ${CLAUDE_PLUGIN_ROOT}/kb/ relevantes para esta feature} | {Quais padrões consultar} |
...
- **Domínios KB** → Fase de Design puxa os padrões corretos de `${CLAUDE_PLUGIN_ROOT}/kb/`
...
> Decidida pela LLM da fase aplicando `${CLAUDE_PLUGIN_ROOT}/sdd/architecture/AGENT_SELECTION_RUBRIC.md` a esta especificação.
