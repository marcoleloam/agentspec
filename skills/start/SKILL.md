...
Glob(sdd/features/*.md)              → Features em andamento
Glob(sdd/reports/*.md)               → Build reports
Glob(sdd/archive/**/*)               → Features entregues
```

### Passo 2: Montar Inventário de Features

Para cada arquivo encontrado em `sdd/features/`:
...
| Brainstorm existe, Define não | `/definir sdd/features/00_BRAINSTORM_{X}.md` |
| Define existe, Design não | `/projetar sdd/features/01_DEFINE_{X}.md` |
| Design existe, Build não | `/construir sdd/features/02_DESIGN_{X}.md` |
